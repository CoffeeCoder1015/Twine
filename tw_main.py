...
class TWINE_interface:
    ...
    def search(self):
        ...
        matchMode = self.ar_input("Match mode (c,<leave empty>):",BREAKER,UNKNOWN_OPTION,["c",""])

        #SEARCH TYPE
        print("f = file, d = folder, <leave empty> = file and folder")
        searchType = self.ar_input("Search type (f,d,<leave empty>):",BREAKER,UNKNOWN_OPTION,["f","d",""])
                ...
                if cmd_in.count("") != len(cmd_in):
                    if cmd_in[0] in cmd_map:
                        ...
                        try:
                            ...
                            elif cmd_in[0] == "open" and len(cmd_in) == 2:
                                cmd_lst[cmd_in[0]](int(cmd_in[1]))
                            else:
                                cmd_lst[cmd_in[0]]()
